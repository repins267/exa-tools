...
The header logo is selectable per render via the spec's `brand`: `exabeam` (default, for customer-facing reports through official Exabeam communications), `exa-tools` (internal, so the reader sees exa-tools produced it), or a customer logo registered once with `exa brand add <name> <file>`. A small "Generated by exa-tools" footer is on by default; pass `footer_brand: false` for a pure-Exabeam deliverable. Everything is base64-embedded, so reports stay self-contained.
