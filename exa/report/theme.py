...
def _brand_footer() -> str:
    """The always-on 'Generated by exa-tools' attribution footer.

    Present on every report so an internal reader knows exa-tools produced it,
    regardless of which brand fills the header. Neutral colour so it reads on
    both the light and dark theme; disable per render with footer_brand=False.
    """
    from importlib.metadata import PackageNotFoundError
    from importlib.metadata import version as _pkg_version

    try:
        ver = _pkg_version("exa-tools")
    except PackageNotFoundError:  # pragma: no cover - installed in all real runs
        ver = ""
    vtxt = f" v{ver}" if ver else ""
    return (
        '<footer style="max-width:1400px;margin:28px auto 44px;padding-top:14px;'
        'border-top:1px solid rgba(127,127,127,0.25);color:#8b95a3;font-size:12px;'
        f'text-align:center">Generated by <b>exa-tools</b>{vtxt}</footer>'
    )
...
def page(
    ...
    brand: which logo fills the header -- 'exabeam' (default, customer-facing),
    'exa-tools' (internal), a registered customer name, or a logo file path. See
    exa.report.brand.resolve_logo.
    footer_brand: append the 'Generated by exa-tools' footer (default True).
    """
    from exa.report.brand import resolve_logo

    logo = resolve_logo(brand)
    logo_html = (
        f'<img class="logo-dark" src="{logo.dark_src}" alt="{_esc(logo.alt)}" />'
        f'<img class="logo-light" src="{logo.light_src}" alt="{_esc(logo.alt)}" />'
    )
    footer_html = _brand_footer() if footer_brand else ""
