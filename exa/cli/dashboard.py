...
def preview_cmd(
    ...
    brand: Annotated[
        str,
        typer.Option("--brand", help="Header logo: exabeam, exa-tools, registered name, or path"),
    ] = "exabeam",
    footer: Annotated[
        bool,
        typer.Option("--footer/--no-footer", help="Append 'Generated by exa-tools' footer [on]"),
    ] = True,
) -> None:
    ...
    try:
        if len(named) == 1 and not path.is_dir():
            ...
            html = dashboard_preview_html(
                cfg, client=client, sample_limit=sample_limit,
                brand=brand, footer_brand=footer,
            )
