...
TOOL_DEFS: list[Tool] = [
    ...
    Tool(
        name="render_report",
        description=(
            "Render a report as a branded, self-contained HTML file (dark by default with a "
            "light toggle, logo, KPI cards, tables) and save it to disk. USE THIS FOR EVERY "
            "REPORT so output is on-brand -- do not hand-write HTML/CSS. Pass a spec: {title, "
            "subtitle?, cards:[{label,value,status?,hint?}] (status is good|warn|bad), "
            "sections:[{title, subtitle?, note?, coverage_pct?, table?:[{col:val,...}]}], "
            "meta?:[str], theme?, brand?, footer_brand?}. brand selects the header logo: "
            "'exabeam' (default -- for customer-facing reports through official Exabeam "
            "communications), 'exa-tools' (internal, so the reader sees exa-tools produced it), "
            "or a customer logo registered with `exa brand add <name>`. footer_brand defaults "
            "true (a small 'Generated by exa-tools' footer); pass false for a pure-Exabeam "
            "customer deliverable. Returns the saved file path; open it or print to PDF."
        ),
        inputSchema={
            "type": "object",
            "required": ["spec"],
            "properties": {
                "spec": {
                    "type": "object",
                    "description": "The report spec (title/cards/sections/meta/brand/footer_brand).",
                },
                "output_path": {
                    "type": "string",
                    "description": "Where to save the HTML [default: reports/{kind}/{tenant}/<title-slug>.html, e.g. reports/customer/<tenant>/...]. Intermediate dirs are created automatically.",
                },
            },
        },
    ),
