...
- Output a **single self-contained HTML file** (no external CSS/fonts, readable in light
  and dark) and also save the markdown source. Reference build: "<tenant> - TAM Report".
- **Branding:** render through `render_report` and choose the header logo with the spec's
  `brand`: **`exabeam`** (default) for a report going to the customer through official
  Exabeam communications; **`exa-tools`** for an internal copy so the reader sees exa-tools
  produced it; or a **registered customer name** (`exa brand add <name> <file>`) to put the
  customer's own logo on their report. A small "Generated by exa-tools" footer is on by
  default — pass `footer_brand: false` for a pure-Exabeam customer deliverable.
